...
from app.models import ScoreBreakdown, Signals
from app.reachability import BitsetReachability, bit, iter_bits, popcount
...
def combine(
    ...
    if signals.is_self_loop:
        ...
        s_loop = min(1, (float(signals.ret_mult) - 1) / cfg.LOOP_SCALE)
    else:
        s_cycle = 0
        s_loop = 0
    ...
    s_reach = min(1, max(0, s_reach))
    s_red = min(1, max(0, s_red))
    s_fan = min(1, max(0, s_fan))
    s_cycle = min(1, max(0, s_cycle))
    s_loop = min(1, max(0, s_loop))
    s_scc = min(1, max(0, s_scc))
    ...
    score = 1 - math.exp(-raw / cfg.SCALE)
    score = round(min(1, max(0, score)), cfg.SCORE_DECIMALS)
