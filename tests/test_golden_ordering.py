...
class GoldenOrderingTests(unittest.TestCase):
    ...
    def test_tighter_cycle_wins_all_else_equal(self):
        """Tighter loop is worse. len 2 must outrank len 3, 5, 7 on equal evidence.

        The evaluator stream ranked len=2 rings below len=5 and len=7 ones
        because flooring the tightness denominator at 3 erased the difference.
        """
        from services.graph_models import Signals
        ...
        def sig(cycle_len):
            return Signals(
                ...
                scc_size=32,
                ret_mult=2,
                is_repeat_edge=False,
                is_self_loop=False,
            )

        scores = [combine(sig(length), CFG).score for length in (2, 3, 5, 7, 10)]
        for tighter, looser in zip(scores, scores[1:]):
            self.assertGreater(tighter, looser)

    def test_large_redundancy_still_orders(self):
        """Evidence must keep ordering past the old clamp, not saturate flat."""
        from services.graph_models import Signals

        def sig(n_red, cycle=True):
            return Signals(
                n_new=0,
                n_red=n_red,
                indeg_v_after=1,
                outdeg_u_after=1,
                cycle_closed=cycle,
                cycle_len=3 if cycle else None,
                scc_size=3 if cycle else 1,
                ret_mult=1 if cycle else 0,
                is_repeat_edge=False,
                is_self_loop=False,
            )

        modest = combine(sig(33), CFG).score
        huge = combine(sig(1520), CFG).score
        self.assertGreater(huge, modest)
        self.assertLessEqual(huge, 1.0)
        self.assertGreater(
            combine(sig(1520, cycle=False), CFG).score,
            combine(sig(33, cycle=False), CFG).score,
        )

    def test_bigger_ring_outranks_smaller_ring(self):
        from services.graph_models import Signals
        ...
        self.assertGreater(combine(sig(37), CFG).score, combine(sig(20), CFG).score)
